...
<body class="bg-slate-50 dark:bg-zinc-950 text-slate-800 dark:text-zinc-100 antialiased min-h-screen flex transition-colors duration-200">
    ...
    <div id="sidebar-backdrop"
         class="fixed inset-0 z-[190] bg-slate-900/50 backdrop-blur-sm hidden lg:hidden"
         onclick="closeSidebar()"></div>

    {{-- ── SIDEBAR ──────────────────────────────────────────────── --}}
    <aside id="sidebar"
           class="fixed lg:static inset-y-0 left-0 z-[200]
                  w-72 lg:w-64
                  bg-white dark:bg-zinc-900
                  border-r border-slate-200/70 dark:border-zinc-800
        ...
        <div class="px-6 py-5 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
            ...
            <button onclick="closeSidebar()"
                    class="lg:hidden w-8 h-8 flex items-center justify-center text-slate-400 dark:text-zinc-500 hover:text-slate-600 dark:hover:text-zinc-300 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800">
        ...
        <nav class="flex-1 px-3 py-5 space-y-0.5 overflow-y-auto">
            ...
            <a href="{{ route('admin') }}" onclick="closeSidebar()"
               class="flex items-center gap-3 py-2 pr-3 rounded-lg text-sm font-medium transition-colors
            ...
            <a href="{{ route('inventory') }}" onclick="closeSidebar()"
               class="flex items-center gap-3 py-2 pr-3 rounded-lg text-sm font-medium transition-colors
    ...
    <!-- Main content — ocupa todo el ancho en móvil ya que sidebar es fixed -->
    <main class="flex-1 min-h-screen overflow-auto min-w-0 w-full">

        <!-- Top bar -->
        <header class="bg-white dark:bg-zinc-900 border-b border-slate-200/70 dark:border-zinc-800 px-4 lg:px-8 py-4 flex items-center justify-between sticky top-0 z-10">
            <div class="flex items-center gap-3 min-w-0">
                {{-- Hamburger — solo móvil --}}
                <button onclick="openSidebar()"
                        class="lg:hidden w-9 h-9 flex items-center justify-center text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg transition-colors flex-shrink-0"
                        aria-label="Abrir menú">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"/>
                    </svg>
                </button>
                <h1 class="text-base font-semibold text-slate-900 dark:text-zinc-50 tracking-tight truncate">@yield('page-title', 'Dashboard')</h1>
            </div>
            <div class="flex items-center gap-2 flex-shrink-0">
                <span class="hidden sm:inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wide
                    {{ auth()->user()->isAdmin()
